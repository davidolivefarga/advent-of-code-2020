...
input[0] = input[0].split(/\n/).reduce((acc, curr) => {
	const [ruleNum, ruleContent] = curr.split(": ");

	if (ruleContent.startsWith('"')) {
		acc[ruleNum] = ruleContent.slice(1, 2);
	} else if (ruleContent.includes("|")) {
		const [r1, r2] = ruleContent.split(" | ");
		acc[ruleNum] = [r1.split(" "), r2.split(" ")];
	} else {
		acc[ruleNum] = ruleContent.split(" ");
	}
