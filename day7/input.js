...
`
	...
	.reduce((bags, line) => {
		if (line.indexOf("contain no other bags") == -1) {
			const i = line.indexOf(" bags contain ");

			const bag = line.slice(0, i);
			const bagContent = line
				.slice(i + 14)
				.split(", ")
				.map((l) => l.replace(/ bags\.| bags| bag\.| bag/, ""))
				.reduce((content, bagData) => {
					const i = bagData.indexOf(" ");

					content[bagData.slice(i + 1)] = parseInt(bagData.slice(0, i), 10);
